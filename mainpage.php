<?php
require "db.php";   // عدّل حسب مكان ملف db.php
session_start();

// جلب كل العقارات مرتّبة من الأحدث للأقدم
...
<head>
  ...
  <title>عقاري | الصفحة الرئيسية</title>
  <style>
:root{
  --bg: #f8fafd;
  --panel: #ffffff;
...
<header class="navbar">
  ...
  <nav class="links">
      <a class="active" href="mainpage.php">الرئيسية</a>
...
<section class="hero">
    <div class="hero-overlay">
      <h1>عقاري</h1>
      <h1>ابحث عن عقارك المناسب بسهولة</h1>
      <p>شقق • فلل • أراضٍ • محلات — في جميع المدن الفلسطينية</p>
      <a href="#filters" class="hero-btn">ابدأ الآن</a>
    </div>
</section>
...
function applyFilters() {
  ...
  cards.forEach(card => {
    ...
    const cArea  = parseInt(card.dataset.area || '0', 10);
    const cText  = normalize(card.dataset.text);

    const okType  = !type || cType === type;
   const okProvince = !province || cProvince === province;
        ...
        const show = okType && okProvince && okCity && okMin && okMax && okArea && okQuery;
